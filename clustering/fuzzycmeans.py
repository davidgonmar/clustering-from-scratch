...
class FuzzyCMeans:
    def _euclidean_distance(self, x1: np.ndarray, x2: np.ndarray) -> float:
        ...
        """
        return np.sqrt(np.sum((x1 - x2) ** 2, axis=-1)) # sum over the features, not the samples
    
    _dist = _euclidean_distance



    def __init__(self, n_clusters: int = 3, max_iter: int = 100, m: float = 2, eps: float = 0.01):
    ...
    def _init_membership(self, n_samples: int) -> np.ndarray:
        ...
        return r / r.sum(0) # normalize the rows to sum to 1 (sum accross a column is 1)
    ...
    def _compute_centroids(self, X: np.ndarray, membership: np.ndarray) -> np.ndarray:
        ...
        """
        return np.dot(membership ** self.m, X) / np.sum(membership ** self.m, axis=1, keepdims=True)
    ...
    def _update_membership(self, X: np.ndarray, centroids: np.ndarray) -> np.ndarray:
        ...
        """
        distances = np.array([self._dist(X, c) for c in centroids]) # shape (n_clusters, n_samples)
        dsum_per_cluster = distances.sum(0, keepdims=True) + 1e-8 # avoid div by zero, shape (1, n_samples)
        normalized_dists = distances / dsum_per_cluster + 1e-8 # avoid div by zero, shape (n_clusters, n_samples)
    ...
    def fit(self, X: np.ndarray):
        ...
        for _ in range(self.max_iter):
            ...
            if np.allclose(new_membership, self.membership, atol=self.eps):
                ...
                self.centroids = self._compute_centroids(X, new_membership) # update centroids one last time
                break
    ...
    def get_hard_labels(self):
        ...
        return np.argmax(self.membership, axis=0) # get the index of the maximum value along the rows